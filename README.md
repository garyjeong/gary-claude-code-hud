...
- **사용량이 0이어도 grok 항목은 표시됩니다.** 창이 막 리셋됐거나 이번 주 grok을 쓰지
  않았으면 `grok 0% 0(주간·초기화시각)`으로 나옵니다. 예전에는 이때 항목이 통째로
  사라져 "안 씀"과 "읽기 실패"를 구분할 수 없었고, `partial` 판정에 걸려 TTL이 10초로
  줄면서 세션 전체 스캔을 반복하는 부작용도 있었습니다. 세션 디렉터리를 아예 읽지 못한
  경우에만 항목을 생략합니다(그게 진짜 실패입니다).
